...
func appendBlogRouters(userHandle *handle.UserHandle, articleHandle *handle.ArticleHandle) []*vortex.VortexHttpRouter {
	return []*vortex.VortexHttpRouter{
		// 用户接口
		vortex.AppendHttpRouter([]string{http.MethodPost}, "/blog/console/login/pwd", userHandle.LoginByPassword, "根据密码登录"),
		vortex.AppendHttpRouter([]string{http.MethodPost}, "/blog/console/login/email_code", userHandle.LoginByEmailCode, "根据邮箱验证码登录"),

		// 文章接口
		vortex.AppendHttpRouter([]string{http.MethodPost}, "/blog/console/article/publish", articleHandle.HandlePublishArticle, "发布文章"),
		vortex.AppendHttpRouter([]string{http.MethodPost}, "/blog/console/article/update/:id", articleHandle.HandleUpdateArticle, "更新文章"),
		vortex.AppendHttpRouter([]string{http.MethodDelete}, "/blog/console/article/delete/:id", articleHandle.HandleDeleteArticle, "删除文章"),
	}
}
